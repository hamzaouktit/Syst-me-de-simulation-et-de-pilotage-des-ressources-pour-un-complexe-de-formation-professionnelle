        ...
        <div class="col-lg-8">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <p class="text-muted">{{ $module->nom }}</p>
                </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('administrationetablissement.modules.update', $module) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" class="form-control @error('nom') is-invalid @enderror" 
                                   id="nom" name="nom" value="{{ old('nom', $module->nom) }}" required>
                            @error('nom')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" class="form-control @error('masse_horaire') is-invalid @enderror" 
                                   id="masse_horaire" name="masse_horaire" value="{{ old('masse_horaire', $module->masse_horaire) }}" 
                                   min="1" required>
                            @error('masse_horaire')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                                        ...
                                        @foreach($formations as $formation)
                                            <div class="col-md-6 mb-2">
                                                <div class="form-check">
                                                    @php
                                                        $isChecked = in_array($formation->id, old('formations', $module->formations->pluck('id')->toArray()));
                                                    @endphp
                                    ...
                                    <div class="row">
                                        @foreach($metiers as $metier)
                                            <div class="col-md-6 mb-2">
                                                <div class="form-check">
                                                    @php
                                                        $isMetierChecked = in_array($metier->id, old('metiers', $module->metiers->pluck('id')->toArray()));
                                                    @endphp
                                                    <input class="form-check-input" type="checkbox" 
                                                           name="metiers[]" value="{{ $metier->id }}" 
                                                           id="metier_{{ $metier->id }}"
                                                           {{ $isMetierChecked ? 'checked' : '' }}>
            ...
            <div class="card mt-4">
                <div class="card-header">
                    <h6 class="card-title mb-0">Associations Actuelles</h6>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            @if($module->formations->count() > 0)
                                <ul class="list-unstyled">
                                    @foreach($module->formations as $formation)
                                        <li><span class="badge bg-primary">{{ $formation->titre }}</span></li>
                                    @endforeach
                                </ul>
                            @else
                            ...
                            @if($module->metiers->count() > 0)
                                <ul class="list-unstyled">
                                    @foreach($module->metiers as $metier)
                                        <li><span class="badge bg-secondary">{{ $metier->nom }}</span></li>
                                    @endforeach
                                </ul>
                            @else
